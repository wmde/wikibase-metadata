"""Get Aggregate External Identifier"""
...
from model.strawberry.output import WikibaseExternalIdentifierAggregateStrawberryModel
from resolvers.util import get_filtered_wikibase_query


async def get_aggregate_external_identifier(
    wikibase_filter: Optional[WikibaseFilterInput],
) -> WikibaseExternalIdentifierAggregateStrawberryModel:
    """Get Aggregate External Identifier"""

    total_external_identifier_query = get_total_external_identifier_query(
        wikibase_filter
    )
    ...
    async with get_async_session() as async_session:
        (
            ...
            wikibase_count,
        ) = (await async_session.execute(total_external_identifier_query)).one()
...
def get_total_external_identifier_query(
    wikibase_filter: Optional[WikibaseFilterInput],
    ...
    query = select(
        ...
        func.sum(WikibaseExternalIdentifierObservationModel.total_url_properties).label(
            "total_url_properties"
        ),
        func.sum(WikibaseExternalIdentifierObservationModel.total_url_statements).label(
            "total_url_statements"
        ),
        # pylint: disable-next=not-callable
