...
async def wikibase_with_extensions_observation(db_session):
    ...
    async with get_async_session() as session:
        extensions = [
            ...
            ("Google Analytics Integration", "3.0.1", datetime(2019, 8, 6, 9, 12), "6441403"),
            ("LabeledSectionTransclusion", "f621799", datetime(2020, 1, 29, 14, 52), "f621799"),
            ...
            ("UniversalLanguageSelector", "2020-01-23", datetime(2020, 3, 3, 13, 38), "61f1a98"),
            ("WikibaseClient", "dbbcdd8", datetime(2019, 12, 10, 12, 52), "dbbcdd8"),
            ("WikibaseLib", "dbbcdd8", datetime(2019, 12, 10, 12, 52), "dbbcdd8"),
            ("WikibaseRepository", "dbbcdd8", datetime(2019, 12, 10, 12, 52), "dbbcdd8"),
...
@pytest.mark.query
@pytest.mark.version
async def test_wikibase_software_version_most_recent_observation_extensions_query(wikibase_with_extensions_observation):
    """Test Wikibase Most Recent Software Version Installed Extensions Observation Query"""
